...
const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'py-2 sm:py-3' : 'py-3 sm:py-5'}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div
            className={`glass rounded-xl sm:rounded-2xl px-3 sm:px-5 py-2 sm:py-3 flex items-center gap-2 sm:gap-4 transition-all duration-500 ${scrolled ? 'shadow-[0_20px_60px_rgba(0,0,0,0.6)]' : 'shadow-[0_10px_30px_rgba(0,0,0,0.3)]'}`}
          >
            {/* Logo */}
            <div
              className="cursor-pointer flex-shrink-0 scale-90 sm:scale-100"
            ...
            <div className="flex-grow max-w-sm lg:flex-none lg:w-56 relative group mx-2">
              <svg
                className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-zinc-600 transition-colors group-focus-within:text-amber-500"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <input
                ...
                placeholder="Search..."
                className="w-full bg-white/90 border border-white/20 rounded-xl py-2 pl-8 pr-8 text-[10px] text-black outline-none focus:border-amber-500/60 focus:bg-white transition-all placeholder:text-gray-500 font-medium"
              />
              {searchQuery && (
                <button
                  onClick={() => onSearch('')}
            ...
            {user && (
              <div className="flex lg:hidden items-center gap-2 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-xl ml-2 overflow-hidden flex-shrink-0">
                <span className="text-[10px] font-bold text-amber-500">
                  {toINRString(user.walletBalance)}
                </span>
              </div>
            )}
            ...
            >
              <span
                className={`block w-4 h-0.5 bg-white rounded-full transition-all duration-300 ${mobileOpen ? 'rotate-45 translate-y-[3px]' : ''}`}
              />
              <span
                className={`block w-4 h-0.5 bg-white rounded-full transition-all duration-300 my-[3px] ${mobileOpen ? 'opacity-0' : ''}`}
              />
              <span
                className={`block w-4 h-0.5 bg-white rounded-full transition-all duration-300 ${mobileOpen ? '-rotate-45 -translate-y-[3px]' : ''}`}
              />
              ...
              {user ? (
                <>
                  <div className="hidden sm:block text-right">
                    <p className="text-[10px] text-white font-bold tracking-wide leading-none">
                      {user.name}
                    </p>
                    <p className="text-[8px] text-amber-500/60 uppercase tracking-widest font-semibold mt-0.5">
                      {user.role}
                    </p>
                  </div>
                  ...
                  <div className="relative group">
                    <div
                      onClick={() => {
                        if (window.innerWidth < 1024) onViewChange('profile');
                      }}
                    ...
                    <div className="hidden lg:block absolute right-0 top-full pt-3 opacity-0 -translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300">
                      <div className="glass rounded-2xl p-4 w-56 shadow-[0_30px_60px_rgba(0,0,0,0.8)] border border-white/10">
                        ...
                        <div className="mb-3 p-3 bg-amber-500/8 border border-amber-500/12 rounded-xl">
                          <p className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1 font-semibold text-center">
                            Wallet Balance
                          </p>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <button
                            onClick={onLogout}
                            className="w-full text-left px-3 py-2.5 bg-red-500/8 hover:bg-red-500/15 text-red-400 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all flex items-center gap-2"
                          >
                            <svg
                              className="w-3.5 h-3.5"
                              fill="none"
      ...
      >
        <div className="flex flex-col h-full p-6 pt-20">
          ...
          <div className="space-y-2">
            <p className="text-[9px] uppercase tracking-[0.3em] text-zinc-500 font-bold mb-4 ml-2">
              Navigation
            </p>
            {navLinks.map((link) => (
              <button
              ...
              >
                ...
                {link.badge && (
                  <span className="text-[8px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full border border-red-500/30">
                    {link.badge}
                  </span>
                )}
              </button>
            ...
            <div className="mt-auto pb-8">
              ...
              <div className="flex items-center gap-3 mb-6 px-2">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-amber-500/60 uppercase tracking-widest font-black">
                    {user.role}
                  </p>
                </div>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/8 mb-4">
                <p className="text-[8px] text-zinc-500 uppercase tracking-widest mb-1 font-bold">
                  Wallet Balance
                </p>
